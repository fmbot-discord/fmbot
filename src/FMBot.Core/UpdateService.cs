...
public class UpdateService
{
    ...
    private readonly IdBackfillService _idBackfillService;
    ...
    {
        if (this._cache.TryGetValue(IndexKeys.IndexConcurrencyCacheKey(user.UserId), out bool _) &&
            !bypassIndexPending)
        {
    ...
    public async Task<Response<RecentTrackList>> UpdateUser(UpdateUserQueueItem queueItem)
    {
        ...
        var user = await this._userLookup.GetUserForIdAsync(queueItem.UserId);
        ...
        try
        {
            ...
            if (queueItem.UpdateQueue && !SupporterRules.IsSupporter(user.UserType))
            {
        ...
        if (queueItem.UpdateQueue)
        {
            CoreStatistics.UpdatedUsers.WithLabels("queue").Inc();
        }
        else
        {
            CoreStatistics.UpdatedUsers.WithLabels("user_init").Inc();
        }
    ...
    private async Task BackfillPlayIds(User user)
    {
        ...
        await this._idBackfillService.BackfillUserPlayIds(user.UserId);
        await this._idBackfillService.BackfillUserArtistIds(user.UserId);
        await this._idBackfillService.BackfillUserAlbumIds(user.UserId);
        await this._idBackfillService.BackfillUserTrackIds(user.UserId);
    }
