...
public class ServerAlbumsRenderer(PlayService playService, WhoKnowsAlbumService whoKnowsAlbumService)
    : IAutopostRenderer
{
    public AutopostType Type => AutopostType.ServerAlbums;
    ...
    public async Task<AutopostRenderResult> RenderAsync(AutopostRenderContext context)
    {
        ...
        List<GuildAlbum> topAlbums;
        if (allTime)
        {
            topAlbums = (await whoKnowsAlbumService.GetTopAllTimeAlbumsForGuild(context.Autopost.GuildId,
                OrderType.Listeners, context.Autopost.ArtistFilter, context.RoleUserIds)).ToList();
        }
        else
        {
            topAlbums = await playService.GetGuildTopAlbumsPlays(context.Autopost.GuildId, context.PeriodStart,
        ...
        {
            Type = this.Type,
            Sections = [AutopostRendering.ToAlbumSection(topAlbums, "Top albums")]
        };

        return new AutopostRenderResult
        {
            Container = AutopostRendering.BuildChartContainer(context, snapshot, "top albums", allTime),
            Snapshot = snapshot,
