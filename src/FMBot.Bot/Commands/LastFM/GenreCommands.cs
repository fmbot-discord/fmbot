...
namespace FMBot.Bot.Commands.LastFM
{
    public class GenreCommands : BaseCommandModule
    {
        ...
        [RequiresIndex]
        [CommandCategories(CommandCategory.Genres)]
        public async Task GuildGenreAsync([Remainder] string extraOptions = null)
        {
            var prfx = this._prefixService.GetPrefix(this.Context.Guild?.Id);
            ...
            try
            {
                ...
                var title = $"Top {guildListSettings.TimeDescription.ToLower()} genres in {this.Context.Guild.Name}";
                ...
                {
                    case 1:
                        footer.AppendLine($"View specific genre listeners with '{prfx}whoknowsgenre'");
                        break;
                    case 2:
